...
Following documentation items shall be included when applicable.
   ...
   4) (Mandatory) The implementation details of this function shall be documented between
      ``@internal`` and ``@endinternal`` tags.
      The principle is that every line of the code inside the function can be derived
      from the description here.

   5) (Mandatory if the function parameter is not void)
   ...
   6) (Mandatory) The description of the return value shall be documented
      with the format ``@return <description of the return value>``.
      A void-returning function shall be documented with ``@return None``.

   7) (Mandatory if the return value represents error conditions)
   ...
   8) (Mandatory) The pre-condition of the function shall be documented with the format
      ``@pre <pre-condition description>``. C expressions shall be used when possible.
      If there is no pre-conditions, say ``@pre N/A``.

   9) (Mandatory) The post-condition of the function shall be documented with the format
      ``@post <post-condition description>``. C expressions shall be used when possible.
      If there is no post-conditions, say ``@post N/A``.

   10) (Optional) A set of remarks that specify the additional constraints on using this function
...
Compliant example::
	 ...
	 *
	 * @internal
	 * The implementation details of this function describes here.
	 * The principle is that every line of the code inside the function can be derived from the description here.
	 * @endinternal
	 *
