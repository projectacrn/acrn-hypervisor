...
:acrn-issue:`2522` - [NUC7i7BNH]After starting ias in SOS, there is no display 
       On NUC7i7BNH, after starting ias in SOS, there is no display. And this issue only occurs on this model NUC which with TPC to VGA connector. 
...
:acrn-issue:`2523` - UOS monitor does not display when using ias
      It fails to use ias weston in UOS. There is no display for UOS after starting ias weston.

   **Impact:** Cannot use ias weston in UOS.

   **Workaround:** 
              1) To use weston instead of ias weson. (swupd install x11-server)
...
:acrn-issue:`2524` - [UP2][SBL] Launching UOS hang while weston is running in SOS
      After using weston in SOS, it will hang during UOS launching.

   **Impact:** launching UOS hang, and then no display in UOS.

   **Workaround:** Use acrn-kernel to rebuild SOS kernel to replace integrated kernel. To confirm "DRM_FBDEV_EMULATION" related configs in kernel_config_sos should as below:
                .. code-block:: bash
...
:acrn-issue:`2527` - [KBLNUC][HV]System will crash when run crashme (SOS/UOS)
      If running crashme tool in SOS/UOS for stress test, system will crash after minutes.
...
:acrn-issue:`2528` - [APLUP2] SBL (built by SBL latest code) failed to boot ACRN hypervisor
   ...
   **Workaround:** Use SBL built by earlier slimbootloader code (commit id:edc112328cf3e414523162dd75dc3614e42579fe) can boot acrn hypervisor normally.
