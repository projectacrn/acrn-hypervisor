...
``SERIAL_CONSOLE`` (a child node of ``DEBUG_OPTIONS``):
  ...
  This configuration is valid only if SOS legacy_vuart0 is enabled.
  Leave this field empty if SOS console_vuart is enabled. Using
...
``bootargs`` (a child node of ``os_config``):
  Specify the kernel boot arguments in bootargs under the parent of board_private.
...
``base`` (a child node of ``legacy_vuart``,``console_vuart``,``communication_vuart``):
...
``target_uart_id`` (a child node of ``legacy_vuart1``,``communication_vuart``):
