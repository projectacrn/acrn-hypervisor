...
static void ptirq_free_irte(const struct ptirq_remapping_info *entry)
{
	...
	if (entry->intr_type == PTDEV_INTR_MSI) {
		...
		intr_src.src.msi.value = entry->phys_sid.msi.bdf;
	} else {
...
ptirq_build_physical_rte(struct acrn_vm *vm, struct ptirq_remapping_info *entry)
{
	...
	vioapic_get_rte(vm, virt_sid->ioapic.gsi, &virt_rte);
	rte = virt_rte;

	/* init polarity & pin state */
	if (rte.bits.intr_polarity == IOAPIC_RTE_INTPOL_ALO) {
		if (entry->polarity == 0U) {
			vioapic_set_irqline_nolock(vm, virt_sid->ioapic.gsi, GSI_SET_HIGH);
		}
		entry->polarity = 1U;
	} else {
		if (entry->polarity == 1U) {
			vioapic_set_irqline_nolock(vm, virt_sid->ioapic.gsi, GSI_SET_LOW);
		}
		entry->polarity = 0U;
	}
	...
	uint16_t virt_bdf, uint16_t phys_bdf, uint32_t entry_nr)
{
	...
	if (entry == NULL) {
		...
		if (is_sos_vm(entry->vm)) {
			entry->vm = vm;
			entry->virt_sid.msi.bdf = virt_bdf;
		} else {
			pr_err("MSIX pbdf%x idx=%d already in vm%d with vbdf%x, not able to add into vm%d with vbdf%x",
				entry->phys_sid.msi.bdf, entry->phys_sid.msi.entry_nr, entry->vm->vm_id,
				entry->virt_sid.msi.bdf, vm->vm_id, virt_bdf);
...
remove_msix_remapping(const struct acrn_vm *vm, uint16_t virt_bdf, uint32_t entry_nr)
{
	...
	if (entry != NULL) {
		...
		dev_dbg(DBG_LEVEL_IRQ,
			...
			entry->phys_sid.msi.bdf, entry_nr);
	...
	if (entry == NULL) {
		...
		if (is_sos_vm(entry->vm)) {
			...
			pr_err("INTX gsi%d already in vm%d with vgsi%d, not able to add into vm%d with vgsi%d",
					phys_gsi, entry->vm->vm_id, entry->virt_sid.ioapic.gsi, vm->vm_id, virt_gsi);
			entry = NULL;
		}
	} else {
		/* The mapping has already been added to the VM. No action
...
static void remove_intx_remapping(const struct acrn_vm *vm, uint32_t virt_gsi)
{
	...
	if (entry != NULL) {
		if (is_entry_active(entry)) {
			...
			dev_dbg(DBG_LEVEL_IRQ,
				"deactive intx entry:pgsi=%d, pirq=%d ",
				entry->phys_sid.ioapic.gsi, phys_irq);
		...
		const struct ptirq_remapping_info *entry)
{
	...
	vioapic_get_rte(vm, (uint32_t)virt_sid->ioapic.gsi, &rte);
	...
	if (trigger_lvl) {
		if (entry->polarity != 0U) {
			vioapic_set_irqline_lock(vm, virt_sid->ioapic.gsi, GSI_SET_LOW);
		} else {
			vioapic_set_irqline_lock(vm, virt_sid->ioapic.gsi, GSI_SET_HIGH);
		}
	} else {
		if (entry->polarity != 0U) {
			vioapic_set_irqline_lock(vm, virt_sid->ioapic.gsi, GSI_FALLING_PULSE);
		} else {
			vioapic_set_irqline_lock(vm, virt_sid->ioapic.gsi, GSI_RAISING_PULSE);
		}
	}
...
int32_t ptirq_intx_pin_remap(struct acrn_vm *vm, uint32_t virt_gsi)
{
	...
	if (!is_vuart_intx(vm, virt_sid.ioapic.gsi)) {
