...
#define ENTRY_HPA1_LOW_HEAD		2U
#define ENTRY_HPA1_LOW_TAIL		4U
...
static const struct e820_entry pre_ve820_template[E820_MAX_ENTRIES] = {
	...
	/* pSRAM segment splits the lowmem into two parts: head and tail */
	{	/* head part of lowmem of hpa1*/
 ...
 *	 entry2: usable, the head part of hpa1 in lowmem, from 0x100000, and up to the bottom of pSRAM area.
 *	 entry3: reserved, pSRAM segment, which will be identically mapped to physical pSRAM segment rather than hpa1.
 *	 entry4: usable, the tail part of hpa1 in lowmem, from the ceil of pSRAM segment, and up to 2G-1M.
...
void create_prelaunched_vm_e820(struct acrn_vm *vm)
{
	...
	uint64_t hpa1_head_max_length = PSRAM_BASE_GPA - MEM_1M;
	...
	if (vm_config->memory.size > lowmem_max_length) {
		...
		entry_idx++;
	} else if (vm_config->memory.size <= MEM_1M + hpa1_head_max_length + MEM_1M) {
		/* in this case, hpa1 is only enough for the first 1M + head + last 1M (ACPI NVS/DATA), so tail part will be empty */
		vm->e820_entries[ENTRY_HPA1_LOW_HEAD].length = vm_config->memory.size - MEM_2M; /* 2M includes the first and last 1M */
		vm->e820_entries[ENTRY_HPA1_LOW_TAIL].length = 0;
	} else {
		/* Otherwise, tail part is not empty. */
		vm->e820_entries[ENTRY_HPA1_LOW_TAIL].length = vm_config->memory.size - PSRAM_BASE_GPA - MEM_1M;
		/* need to set gpa_start for hpa2 */
	}

	hpa2_lo_size = (lowmem_max_length - vm_config->memory.size);
	gpa_start = vm->e820_entries[ENTRY_HPA1_LOW_TAIL].baseaddr + vm->e820_entries[ENTRY_HPA1_LOW_TAIL].length;
