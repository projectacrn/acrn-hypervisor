...
struct hc_dispatch {
	 ...
	 *
	 * - If permission_flags == 0UL (which is the default value), this hypercall can only be invoked by the Service VM.
...
int32_t vmcall_vmexit_handler(struct acrn_vcpu *vcpu)
{
	 ...
	 *
	 * 1. Only Service VM and User VMs with specific guest flags (referred to as 'allowed VMs' hereinafter) can invoke
