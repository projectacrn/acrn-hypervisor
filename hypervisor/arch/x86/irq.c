...
static inline void fixup_idt(struct host_idt_descriptor *idtd)
{
	...
	for (i = 0; i < HOST_IDT_ENTRIES; i++) {
		entry = idt_desc[i].fields.offset_63_32 | ((uint64_t)idt_desc[i].fields.rsvd << 32U);
		idt_desc[i].fields.rsvd = 0U;
		idt_desc[i].fields.offset_63_32 = (uint32_t)(entry >> 32U);
		idt_desc[i].fields.high32.bits.offset_31_16 = (uint32_t)((entry & 0xffffffffUL) >> 16U);
		idt_desc[i].fields.low32.bits.offset_15_0 = (uint32_t)(entry & 0xffffUL);
	}
}
