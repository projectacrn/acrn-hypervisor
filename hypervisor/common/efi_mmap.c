...
{
	struct efi_memory_desc *efi_memdesc = (struct efi_memory_desc *)efi_info->memmap;
	uint32_t entry = 0U;

	while ((void *)efi_memdesc < (efi_info->memmap + efi_info->memmap_size)) {
		 ...
		 */
		efi_memdesc = (struct efi_memory_desc *)((void *)efi_memdesc + efi_info->memdesc_size);
		entry ++;
	}
