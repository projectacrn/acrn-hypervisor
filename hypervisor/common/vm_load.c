...
static uint64_t create_zero_page(struct acrn_vm *vm, uint64_t load_params_gpa)
{
	...
	if (is_sos_vm(vm)) {
		...
		if (boot_from_uefi(abi)) {
			...
			sos_efi_info->efi_memdesc_size = sizeof(struct efi_memory_desc);
			sos_efi_info->efi_memmap_size = efi_mmap_desc_nr * sizeof(struct efi_memory_desc);
			sos_efi_info->efi_memmap = (uint32_t)efi_mmap_gpa;
			sos_efi_info->efi_memmap_hi = (uint32_t)(efi_mmap_gpa >> 32U);
